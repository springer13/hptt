#ifndef STRANSPOSE_3021_96X96X96X96_H
#define STRANSPOSE_3021_96X96X96X96_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
/**
 * B(i3,i0,i2,i1) <- alpha * A(i0,i1,i2,i3) + beta * B(i3,i0,i2,i1);
 */
template<int size0, int size1, int size2, int size3>
void sTranspose_3021_96x96x96x96( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
      ...
      for(int i1 = 0; i1 < size1; i1+= 1)
         for(int i2 = 0; i2 < size2; i2+= 1)
            for(int i3 = 0; i3 < size3 - 15; i3+= 16)
               sTranspose16x16(&A[i0 + i1*lda1 + i2*lda2 + i3*lda3], lda3, &B[i0*ldb1 + i1*ldb3 + i2*ldb2 + i3], ldb1  ,reg_alpha ,reg_beta);
   //Remainder loop
   for(int i0 = size0 - remainder0; i0 < size0; i0 += 1)
      for(int i1 = 0; i1 < size1; i1 += 1)
         for(int i2 = 0; i2 < size2; i2 += 1)
            for(int i3 = 0; i3 < size3 - remainder3; i3 += 1)
               B[i0*ldb1 + i1*ldb3 + i2*ldb2 + i3] = alpha*A[i0 + i1*lda1 + i2*lda2 + i3*lda3] + beta*B[i0*ldb1 + i1*ldb3 + i2*ldb2 + i3];
   //Remainder loop
